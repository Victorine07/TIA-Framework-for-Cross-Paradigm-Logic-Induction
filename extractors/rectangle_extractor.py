...
class RectangleExtractor(BaseCipherExtractor):
    ...
    - T1: Constants                 (sizes, S-box, row rotations, round constants)
    - T2: Primitives                (SubColumn layer, ShiftRow layer, inverses)
    ...
    def get_variant_prefix(self) -> str:
        """Isabelle/Python prefix for this variant, e.g. `rectangle_64_80`."""
        return f"rectangle_{self.block_size}_{self.key_size}"
    ...
    def extract_t1_constants(self) -> List[Dict[str, Any]]:
        ...
        specs = [
            ...
            dict(
                semantic_name="row_rotations",
                python_symbol="ROW_ROTATIONS",
                isabelle_names=[f"{prefix}_row_rotations"],
                type="Row Rotation Amounts Constant",
            ),
            dict(
                semantic_name="rc",
                python_symbol="RC",
    ...
    def extract_t2_primitives(self) -> List[Dict[str, Any]]:
        ...
        specs = [
            dict(
                type="SubColumn Layer Accumulator (Recursive)",
                semantic_name="sub_column_acc",
                python_names=[f"{prefix}_sub_column_acc"],
                isabelle_names=[f"{prefix}_sub_column_acc"],
                group="substitution",
                difficulty="hard",
            ),
            dict(
                type="SubColumn Layer",
                semantic_name="sub_column",
                python_names=[f"{prefix}_sub_column"],
                isabelle_names=[f"{prefix}_sub_column"],
                group="substitution",
                difficulty="easy",
            ),
            dict(
                type="SubColumn Layer Inverse Accumulator (Recursive)",
                semantic_name="sub_column_inv_acc",
                python_names=[f"{prefix}_sub_column_inv_acc"],
                isabelle_names=[f"{prefix}_sub_column_inv_acc"],
                group="substitution",
                difficulty="hard",
            ),
            dict(
                type="SubColumn Layer Inverse",
                semantic_name="sub_column_inv",
                python_names=[f"{prefix}_sub_column_inv"],
                isabelle_names=[f"{prefix}_sub_column_inv"],
                group="substitution",
                difficulty="easy",
            ),
            dict(
                type="ShiftRow Layer",
                semantic_name="shift_row",
                python_names=[f"{prefix}_shift_row"],
                isabelle_names=[f"{prefix}_shift_row"],
                group="permutation",
                difficulty="medium",
            ),
            dict(
                type="ShiftRow Layer Inverse",
                semantic_name="shift_row_inv",
                python_names=[f"{prefix}_shift_row_inv"],
                isabelle_names=[f"{prefix}_shift_row_inv"],
                group="permutation",
                difficulty="medium",
            ),
        ]
    ...
    def extract_t3_structural_components(self) -> List[Dict[str, Any]]:
        ...
        specs = [
            dict(
                type="Key to Rows",
                semantic_name="key_to_rows",
                python_names=[f"{prefix}_key_to_rows"],
                isabelle_names=[f"{prefix}_key_to_rows"],
                group="key_schedule",
                difficulty="easy",
            ),
            dict(
                type="S-box Columns Accumulator (Recursive)",
                semantic_name="sbox_columns_acc",
                python_names=[f"{prefix}_sbox_columns_acc"],
                isabelle_names=[f"{prefix}_sbox_columns_acc"],
                group="key_schedule",
                difficulty="hard",
            ),
            dict(
                type="Apply S-box to Columns",
                semantic_name="apply_sbox_to_columns",
                python_names=[f"{prefix}_apply_sbox_to_columns"],
                isabelle_names=[f"{prefix}_apply_sbox_to_columns"],
                group="key_schedule",
                difficulty="easy",
            ),
            dict(
                type="Key Rows Update",
                semantic_name="update_key_rows",
                python_names=[f"{prefix}_update_key_rows"],
                isabelle_names=[f"{prefix}_update_key_rows"],
                group="key_schedule",
                difficulty="medium",
            ),
            dict(
    ...
    def get_transformation_rules(self, component_type: str) -> List[str]:
        ...
        if "column" in lowered or "row" in lowered:
    ...
    def create_instruction(
        ...
        base = (
            f"Translate the RECTANGLE-{self.block_size}/{self.key_size} {template.get('type')} "
