...
class GiftExtractor(BaseCipherExtractor):
    # Cipher-specific implementation of Algorithm 1 (Section 3: Tiered Isomorphic Alignment - Dataset Construction and Registry) for the GIFT
    # family (SPN). Produces T1–T4 aligned Python ↔ Isabelle/HOL component pairs.
    """
    Source-aligned GIFT extractor with explicit tier metadata.

    Tiers:

    - T1: Constants                 (sizes, S-box, P-box, round constants)
    ...
    def get_variant_prefix(self) -> str:
        """Isabelle/Python prefix for this variant, e.g. `gift_64_128`."""
        return f"gift_{self.block_size}_{self.key_size}"
    ...
    def extract_t3_structural_components(self) -> List[Dict[str, Any]]:
        ...
        specs = [
            dict(
                ...
                semantic_name="update_key_state",
                python_names=[f"{prefix}_update_key_state"],
                isabelle_names=[f"{prefix}_update_key_state"],
                group="key_schedule",
                difficulty="hard",
            ),
            dict(
    ...
    def extract_t4_orchestration_components(self) -> List[Dict[str, Any]]:
        ...
        specs = [
            dict(
                type="Round Constant Mask",
                semantic_name="round_constant_mask",
                python_names=[f"{prefix}_round_constant_mask"],
                isabelle_names=[f"{prefix}_round_constant_mask"],
    ...
    def transformation_to_hint(self, transformation: str) -> str:
        hint_map = {
            ...
            "loop_to_recursion": "Convert iterative loops to recursive functions (or fold).",
        }
    ...
    def create_instruction(
        ...
        base = (
            f"Translate the GIFT-{self.block_size}/{self.key_size} {template.get('type')} "
